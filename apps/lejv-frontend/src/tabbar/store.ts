...
const tabbarList = computed(() =>
  baseTabbarList.filter((item) => hasRequiredRoles(item, userRoles.value)),
);
...
const curPath = ref<string>(
  normalizeRoutePath(uni.getStorageSync(TABBAR_PATH_STORAGE_KEY)) || HOME_PAGE,
);
...
const tabbarStore = reactive({
  ...
  restorePrevIdx() {
    if (prevPath.value === curPath.value) return;
