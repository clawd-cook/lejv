...
<template>
  <view class="container">
    <input v-model="inputValue" placeholder="请输入内容" @input="handleInput" />
...
<template>
  <view class="container">
    ...
    <input type="text" :password="!showPassword" placeholder="显示/隐藏密码" v-model="password2" />
    <button @click="showPassword = !showPassword">{{ showPassword ? "隐藏" : "显示" }}密码</button>
  </view>
</template>
...
<template>
  <view class="container">
    <input v-model="value" placeholder="最多输入10个字符" maxlength="10" @input="handleInput" />
...
<template>
  <view class="container">
    <input ref="input" v-model="value" placeholder="点击按钮获取焦点" :focus="isFocused" />
...
<template>
  <view class="container">
    <input v-model="email" type="text" placeholder="请输入邮箱" @blur="validateEmail" />
